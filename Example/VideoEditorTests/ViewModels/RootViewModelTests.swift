...
struct RootViewModelTests {
    ...
    @Test
    func startEditorSessionWithAnImportedSourceDefersTheResolvedSourceURL() {
        let viewModel = RootViewModel()
        let importedSource = VideoEditorSessionSource.importedFile(
            .init(taskIdentifier: "root-view-model-test") {
                URL(fileURLWithPath: "/tmp/root-view-model.mp4")
            }
        )

        viewModel.startEditorSession(with: importedSource)

        #expect(viewModel.currentSourceVideoURL == nil)
        #expect(
            viewModel.editorDestination?.session
                == .init(source: importedSource)
        )
    }
...
@Suite("AppShellTranscriptionConfigurationTests")
struct AppShellTranscriptionConfigurationTests {

    // MARK: - Public Methods

    @Test
    func resolvedOpenAIAPIKeyTrimsWhitespaceFromInfoDictionary() {
        let apiKey = AppShellTranscriptionConfiguration.resolvedOpenAIAPIKey(
            infoDictionary: [
    ...
    @MainActor
    @Test
    func appShellUsesOpenAIWhisperAsTheDefaultTranscriptionConfiguration() {
