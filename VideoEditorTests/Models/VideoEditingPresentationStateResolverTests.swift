...
struct VideoEditingPresentationResolverTests {
    ...
    func selectedCropPresetPrefersExplicitCanvasPresetMapping() {
        let selectedPreset = VideoEditorKit.VideoEditingPresentationStateResolver.selectedCropPreset(
    ...
    func resolveFiltersUnavailableToolsAndMissingRecordedAudio() async {
        let resolvedState = await VideoEditorKit.VideoEditingPresentationStateResolver.resolve(
    ...
    func resolveCanvasSnapshotSynthesizesLegacySocialPresetState() async {
        let resolvedSnapshot = await VideoEditorKit.VideoEditingPresentationStateResolver.resolveCanvasSnapshot(
            from: VideoEditingConfiguration(
                crop: .init(
                    freeformRect: .init(
                        x: 0.341796875,
    ...
    func selectedLegacyCropPresetReturnsOriginalWhenThereIsNoLegacyRect() {
        let preset = VideoEditorKit.VideoEditingPresentationStateResolver.selectedLegacyCropPreset(
            from: nil,
            referenceSize: CGSize(width: 1920, height: 1080)
        )
